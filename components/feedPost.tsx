...
export function FeedPost({ setFeedPosts }: FeedPostProp) {
            ...
            <CardContent className="grid gap-4 mt-5 ">
                ...
                <div className="grid gap-2 bg-[#191920] p-3 h-auto rounded-lg">
                    <div className="flex">
                        ...
                        <Textarea className="bg-transparent border-none outline-none text-[#7F8084] placeholder:text-[#7F8084]" placeholder="How are you feeling today?" onInput={(e: React.ChangeEvent<HTMLTextAreaElement>) => setPost(e.target.value)} value={post} />
                    </div>
                </div>
            </CardContent>
            <CardFooter className="text-xs font-medium flex justify-end ">
                <Button className="w-28 h-11 bg-btnBlue text-base font-normal" onClick={handlePost}>Post</Button>
            </CardFooter>
        </Card >
    )
}
